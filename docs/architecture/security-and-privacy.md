...
Future subprocess adapters must use versioned structured messages, fixed executable configuration (never a shell), timeouts, bounded stdin/stdout/stderr, cancellation, and explicit content disclosure. Resource limits, symlink policy hardening, export redaction profiles, and malicious database recovery remain tracked risks.
