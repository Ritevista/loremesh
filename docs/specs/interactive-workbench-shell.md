...
## Goals
Provide a dominant primary work surface, persistent evidence-lineage rail, independently scrollable investigation timeline, persistent input, visible focus, provider-neutral slash commands, command history, and structured active-view exports suitable for reuse.
...
The layout contains a context header, dominant primary work surface, compact evidence-lineage rail, independently scrollable investigation timeline, input row, and contextual status row. Content is rendered once in the primary surface rather than duplicated in a detail pane. Tab and Shift-Tab cycle only the three visible interactive regions: primary, timeline, and input. Focused regions differ by border, semantic focus color, and bold weight. `/` enters command input from any region. Enter executes non-blank command input; on a selected knowledge-search row it opens the canonical artifact. `b` restores the previous primary view, including the search-results table. `f` on a focused text document turns the composer into a plain in-document Find field; results are reported non-destructively and the document remains visible. New investigation events automatically reveal the latest timeline entry; users may still focus and scroll historical entries independently. Up/Down traverse rows in a focused table and history while command input is focused. Page Up/Down and Home/End operate on the focused scrollable region. Escape cancels Find, clears/leaves input, or returns focus to the primary surface; Escape never terminates LoreMesh, even when pressed repeatedly. `q` quits only outside input, while `/quit` and `/exit` provide explicit command exits. The footer lists only shortcuts supported by the focused view.
